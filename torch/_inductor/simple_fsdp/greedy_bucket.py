    ...
    for idx, node in enumerate(snodes):
        if get_node_type(node) == NodeType.ALL_GATHER: 
            enable_bucket = forward_greedy(current_comp, current_ag, current_mem, 
                ...
                node_dep_dict[node]["MEMORY"], 160, first_AG)
            first_AG = False
            if enable_bucket:
                # merge all gather
                merged_all_gather, ag_buffer = merge_allgather(sched, all_gather_list)
                merged_wait = merge_ag_wait(sched, ag_wait_list, all_gather_list, ag_buffer)
    ...
    for node in snodes:
        ...
        if get_node_type(node) == NodeType.REDUCE_SCATTER:
            ...
            if all_gather is None:
                ...
                node_dep_dict[all_gather]["RS_TIME"] += run_time
    ...
    for idx, node in enumerate(snodes):
        if get_node_type(node) == NodeType.ALL_GATHER: 
            enable_bucket = backward_greedy(current_comp, current_ag, current_rs, current_mem, 
                node_dep_dict[node]["COMPUTE_TIME"], node_dep_dict[node]["AG_TIME"], 
                node_dep_dict[node]["MEMORY"], node_dep_dict[node]["RS_TIME"], 320, first_AG)
            first_AG = False
            ...
            if enable_bucket:
                ...
                rs_wait_dep_list = node_dep_dict[node]["RS_WAIT_DEP"]
                ...
                rs_wait_dep_list.extend(node_dep_dict[node]["RS_WAIT_DEP"])
